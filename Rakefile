...
BASH
  ...
  task :protoc => ['install_protoc'] do
    sh <<-EOF
      ...
      echo "Generating manifest proto"
      PATH=/tmp/gogo-bin-v1.0.0 #{protoc_binary} --proto_path=$GOPATH/src:#{gogo_dir}/src:. --gogofast_out=$GOPATH/src proto/manifest/manifest.proto
