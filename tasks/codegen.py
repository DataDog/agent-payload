...
# These .proto can be imported by other repositories (not only generated code). To have a consistent import path, we use the GOPATH and we add a /v5/ folder as declared packaged in `go.mod` is ../v5
        ...
        GOPATH={toolchain_dir} GOBIN={toolchain_bin_dir} go install github.com/DataDog/protoc-gen-gostreamer@3850c0c
...
# Install protoc-gen-go
        ...
        echo "Generating metrics v3 proto"
        PATH={toolchain_bin_dir} {protoc_binary} --proto_path={toolchain_include_dir}:. --go_out=$GOPATH/src proto/metrics/intake_v3.proto
